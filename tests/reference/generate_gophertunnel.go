...
// 4794743244247a9395360bced1f294d4882c4df0 (protocol 2168, Minecraft 26.44).
// Run from that checkout: go run /path/to/this/file.go /path/to/gophertunnel-2168.txt
package main
...
func main() {
	if p.CurrentProtocol != 2168 || p.CurrentVersion != "1.26.44" {
	...
	defer out.Close()
	fmt.Fprintln(out, "# gophertunnel 4794743244247a9395360bced1f294d4882c4df0; name packet-id body-hex")
	...
	emit("start_game", &pk.StartGame{EntityUniqueID: -123, EntityRuntimeID: 456, EducationEditionOffer: 2,
		PlayerPermissions: 2, ForceExperimentalGameplay: p.Option(false), GameVersion: "1.26.40",
	...
	emit("delta_full", &pk.MoveActorDelta{EntityRuntimeID: 17, PositionX: p.Option(float32(4)), PositionY: p.Option(float32(-9)),
		...
		RotationYHead: p.Option(float32(270)), OnGround: true, ForceMove: true, ForceMoveLocalEntity: true, ForceCompletion: true})
	...
	emit("input_transaction", &pk.PlayerAuthInput{InputData: p.NewInputFlagsFromIDs(pk.InputFlagCount, []int32{34}),
		ItemInteractionData: p.Option(p.UseItemTransactionData{LegacyRequestID: -2, LegacySetItemSlots: p.Option([]p.LegacySetItemSlot{}),
			Actions: p.Option([]p.InventoryAction{}), ActionType: 1, BlockPosition: p.BlockPos{1, 2, 3}})})
	...
	emit("dimension_custom", &pk.DimensionData{Definitions: []p.DimensionDefinition{{Name: "custom:test", Range: [2]int32{320, -64}, Generator: 1, DimensionType: 1000, PackID: id}}})
	...
	emit("command_2168", &pk.CommandRequest{CommandLine: "say test", Version: "clonepartialbedblockfix"})
	...
	emit("play_sound", &pk.PlaySound{SoundName: "note.harp", LoopCount: -1, Handle: p.Option(uint64(0x1122334455667788))})
	emit("sound_update", &pk.ClientboundUpdateSoundData{ServerSoundHandle: 19,
		Fade:  p.Option(p.SoundDataUpdate{Type: p.SoundDataUpdateFade, Duration: 1.25, TargetVolume: 0.5}),
		Pause: p.Option(p.SoundDataUpdate{Type: p.SoundDataUpdatePause})})
	...
	emit("score_mixed_hotfix4", &pk.SetScore{Entries: []p.ScoreboardEntry{
	...
	emit("diagnostics_empty", &pk.ServerBoundDiagnostics{})
